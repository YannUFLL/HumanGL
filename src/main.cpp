...
int main()
{
    ...
    while (!glfwWindowShouldClose(win))
    {
        ...
        if (kQ && !prevQ)
        {
            params.upperArmL = clamp(params.upperArmL + 0.05f, 0.2f, 2.0f);
        ...
        if (kA && !prevA)
        {
            params.thighL = clamp(params.thighL + 0.05f, 0.2f, 2.0f);
        ...
        if (kZ && !prevZ)
        {
            params.armR = clamp(params.armR + 0.02f, 0.05f, 0.6f);
