...
void CharacterRenderer::draw(float t, AnimMode mode, bool paused) {
    ...
    ms.push(); { // frame du torse (centre)
        // Torse
        ms.push(); { placeTorso(ms); renderPart(ms, C_.torso); } ms.pop();

        // Tête (enfant direct du torse)
        ms.push(); { placeHeadFromTorso(ms); renderPart(ms, C_.head); } ms.pop();
        ...
        ms.push(); {
            ...
            // Haut du bras
            ms.push(); { placeUpperArm(ms); renderPart(ms, C_.arm); } ms.pop();
            // Avant-bras
            ms.push(); { placeForearm(ms, elbow); renderPart(ms, C_.arm); } ms.pop();
        } ms.pop();
        ...
        ms.push(); {
            ...
            ms.rotate(+hip, {1, 0, 0});
            ms.push(); { placeThigh(ms);      renderPart(ms, C_.leg); } ms.pop();
            ms.push(); { placeShin(ms, knee); renderPart(ms, C_.leg); } ms.pop();
        } ms.pop();
